...
export default class MathFighterScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("player", "public/images/Raider_1_Spriteslist.png", {
      frameWidth: 80,
      frameHeight: 80,
    });
    this.load.spritesheet("enemy", "public/images/Raider_2_Spritelist.png", {
      frameWidth: 80,
      frameHeight: 80,
    });
  ...
  create() {
    ...
    // Add overlap detection between slash and player
    this.physics.add.overlap(this.slash, this.player, () => {
      this.spriteHit(this.slash, this.player);
    });

    // Add overlap detection between slash and enemy
    this.physics.add.overlap(this.slash, this.enemy, () => {
      this.spriteHit(this.slash, this.enemy);
    });
    // Start Button
  ...
  createAnimation() {
    ...
    this.anims.create({
      key: "enemy-attack",
      frames: this.anims.generateFrameNumbers("enemy", { start: 28, end: 31 }),
      frameRate: 10,
    });
    this.anims.create({
      key: "enemy-hit",
    ...
    this.anims.create({
      key: "enemy-die",
      frames: this.anims.generateFrameNumbers("enemy", { start: 58, end: 63 }),
      frameRate: 10,
    });
  }
  ...
  gameStart() {
    ...
    this.timerText = this.add.text(300, 20, `Time: ${this.countdownTimer}`, {
      fontSize: "28px",
      fill: "#000",
    });
